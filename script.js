    ...
    function onSuccess(position){
        ...
        fetch(`https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=c5f83168eea142ce99e972491b7ba740`)
        ...
        .then(result =>{
            let allDetails = result.results[0].components; //passing components object to allDetails variable
            console.table(allDetails);
            let { state, postcode, country} = allDetails; //getting country, postcode, country properties value from allDetails obj
            button.innerText = `${state} ${postcode}, ${country}`; //passing these value to the button innerText
